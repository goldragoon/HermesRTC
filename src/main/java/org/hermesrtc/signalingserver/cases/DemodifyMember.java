...
@Component(Signals.DEMODIFY_HANDLER)
public class DemodifyMember implements SignalHandler {
    ...
    private Conversation findConversationToDemodify(InternalMessage message) {
	...
	public void execute(InternalMessage context) {
		...
		Conversation conversation = findConversationToDemodify(context);
		log.info(context.getFrom().getId() + " -> "+ context.getTo().getId());
		conversation.depriveModification(context.getFrom(), context.getTo());

...
@Component(Signals.DELEGATE_HANDLER)
public class DelegateConversationManager implements SignalHandler {
    ...
    private Conversation findConversationToDelegate(InternalMessage message) {
	...
	public void execute(InternalMessage context) {
		...
		Conversation conversation = findConversationToDelegate(context);
		log.info(context.getFrom().getId() + " -> "+ context.getTo().getId());
        conversation.delegate(context.getFrom(), context.getTo());
